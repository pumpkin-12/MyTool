...
| 工具 | 说明 |
| --- | --- |
| **实习日志**（internlog） | 按日期记录工作内容，支持心情、标签、备注、配图（粘贴 / 拖入 / 选文件），**结构化检索**（关键词 × 标签 × 日期 × 效率），贡献度热力图 + **效率趋势曲线**；正文可切 **Markdown 预览**；可导出 **Excel 台账**、周报 **.md / .html（Word 可直接打开）**，并**自动补齐缺失周报**；周报支持 **AI 润色**（需自备 API Key，本项目唯一会联网的功能） |
...
- **新增 AI 功能（第一档）：实习日志的「AI 润色成周报」**。这是本项目**第一个也是目前唯一一个会联网的功能**，实现上刻意做了几件事：
  - **架构**：走 Rust 侧转发（新增 `src-tauri/src/ai.rs`）。不是"更优选择"而是唯一选择 —— 严格 CSP 的 `connect-src 'self' ipc:` 让前端根本连不出去
  - **接口按 OpenAI 兼容格式**（`POST {endpoint}/chat/completions`），OpenAI / DeepSeek / 智谱 / 月之暗面 / 各类中转站都能直接用
  - **API Key 的隔离**：Key 只落 `data/ai/secret.json`（`AppStore` 不管理的目录，物理上不可能被「导出全部数据」带走）；`ai_config_get` 只回 `has_key` + 尾 4 位，**给前端的结构体里根本没有 `api_key` 字段**；`exportAll()` 另加一道 `NEVER_EXPORT` 闸门兜底
  - **首次调用会弹隐私确认**，明示"把 N 篇日志正文发送到 你的接口地址"，确认过一次才不再打扰
  - **一次调用产出四段**：润色正文 / 本周成果 / 待办 / 状态点评。刻意**不用 JSON 输出**（模型多写一句"好的，以下是…"就解析崩），改用固定【】小标题分段；解析做了容错，且**原文无条件可查看**
  - 结果只能「填入周报框」，不自动覆盖 —— 之后复用现有的存 .md / 存 HTML / 复制周报 / 缺失周报标记
  - ⚠️ `Cargo.toml` 里 reqwest **必须** `default-features = false` 并选 `native-tls`：reqwest 0.13 的 `default-tls` 是 rustls → aws-lc-rs，在 Windows 上要 cmake + NASM，保留 default 会编不过
- 测试：新增 `tools/verify-ai.js`（**93 项**：AiCore 抽进 vm 真跑 + Rust 侧文本级断言 + Cargo.toml TLS 选型 + 前端接线）。回归脚本 **14 个、945 项**断言（node 侧 8 个 727 项 · 真机侧 6 个 218 项）。
